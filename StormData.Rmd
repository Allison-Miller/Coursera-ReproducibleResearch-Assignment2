...
PROPDMGEXP (Abbreviation, assume it is an exponent, make into character, telling us how much to multiply the PROPDMG variable by to get actual cost in $US)  
CROPDMG (Crop damage, numeric, needs adjusting with CROPDMGEXP to get actual cost)  
CROPDMGEXP (Abbreviation, assume it is an exponent, make into character, assume it is telling us how much to multiply the CROPDMG variable by to get actual cost in $US). 
...
sessionInfo()
```
perDay3$timeofDay=sprintf("%04d", perDay3$interval)
perDay3$timeofDay=strptime(perDay3$timeofDay, "%H%M")
head(timeOfDay)